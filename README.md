# 锦囊 (Scrip)

> 让便携设备的文字输入更简单 —— 本地优先、不采集数据的一键输入工具。

锦囊把你常用的文字（地址、话术、账号信息、模板、链接等）分类收好，
在任意 App 的输入框里通过自定义键盘一键输入，无需手打或反复复制粘贴。
所有数据只保存在设备本地，不上传、不采集；默认离线，仅在你开启「网址处理」时访问你填写的网址。

> 本仓库（`Scrip-release`）用于公开锦囊的发行说明与公开文档（隐私政策、许可协议、更新日志等），
> 便于用户查验「本地优先、不采集数据」的隐私承诺，并跟踪版本发布情况。

## ✨ 功能特性

- 📁 分类收纳：按场景把常用文字归类管理
- ⌨️ 自定义键盘：任意输入框一键输入，告别手打
- 🔗 链接处理：网址自动识别、清理与规整
- 📊 使用统计：完全本地、不含隐私的使用计数
- 🔒 隐私优先：只存本机、默认离线、不采集、键盘不记录
- 🌍 多语言：English / 简体中文 / 繁體中文 / 日本語 / 한국어 / Español / Deutsch / Français

## 🗂 目录结构

- `docs/` — 隐私政策等公开文档
- `CHANGELOG.md` — 版本更新日志
- `LICENSE` — 开源许可协议

## 🔐 隐私说明

锦囊不收集、不上传任何用户数据，所有内容仅保存在设备本地。
详见 [隐私政策（简体中文）](docs/PrivacyPolicy.zh-CN.md) / [Privacy Policy (English)](docs/PrivacyPolicy.en.md)。

## 📄 许可协议

本项目基于 MIT 许可证开源，详见 [LICENSE](LICENSE)。
